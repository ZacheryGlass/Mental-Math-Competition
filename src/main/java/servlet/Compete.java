	...
	try
	{
		...
		if ( session.isNew() )
		{
			...
			else 
			{
				...
				out.print  (  "    <hr>\n"
						 ...
						 +	  "        <tr style='font-size:x-large;'><td>Sorry, that username is already taken.</td></tr>\n"
		...
		if (isUserLoggedIn)
		{
			...
			while ( easyQuestionsResultSet.next() ) 
			{
			    ...
			    if (isCorrect == null) {
            		out.println ( "         	<li><a  class='not-active' data-target='/Compete' href='/Compete?Q_ID="+ tempQuestionID +"'>"+ tempQuestionText +"</a></li>");
			    } else {
			    	if (isCorrect) {
            			out.println ( "         	<li class='bg-success disabled'><a class='not-active'>"+ tempQuestionText +"</a></li>");
				    } else {
            			out.println ( "         	<li class='bg-danger disabled'><a class='not-active'>"+ tempQuestionText +"</a></li>");
				    }// end inner if else
			    }// end outer if-else
			...
			out.println ( "    <li class='dropdown'>");
            out.println ( "        <a href='#' class='dropdown-toggle' data-toggle='dropdown' role='button' aria-haspopup='true' aria-expanded='false'>Difficult <span class='caret'></span></a>");
            out.println ( "        <ul class='dropdown-menu'>");
			...
			String numOfQuestionsCompletedQuery = "SELECT Count(Question_ID) AS numOfCompletedQuestions FROM Math.Submission WHERE Competitor_ID = " + currentCompetitorID + ";";
			...
			{
				numOfQuestionsCompleted = Integer.parseInt( "" + numOfQuestionsCompletedRS.getObject("numOfCompletedQuestions") ) ;
			}
			...
			if (currentQuestionID == -1) {
				...
				out.println("            <h2 class='display-3'>There are no more questions to answer.</h2>" ) ;
				out.println("            <hr class='my-4'>" ) ;
				out.println("            <p class='lead'>Thank you for participating.</p>" ) ;
				out.println("            <hr class='my-4'>" ) ;
				...
				while (true)
					{
						if (nextQuestionID > 30) // there are 30 questions
							nextQuestionID = 1 ;

						String nextQuestionQuery = "SELECT * FROM Math.Submission WHERE Competitor_ID = " + currentCompetitorID + "  AND Question_ID =  " + nextQuestionID + " " ;
						ResultSet nextQuestionRS = statement.executeQuery(nextQuestionQuery);
						
						if ( ! nextQuestionRS.next() )
							break; // breaking out when user has completed question 30 because of line 466

						nextQuestionID++;// go to next question
				...
				// this loops threw each choice and saves the choice HTML to choices ArrayList 
				while ( choiceResultSet.next() )
				{
					...
					String choiceHTML = "      <tr>";
					choiceHTML += "          <td>\n";
					choiceHTML += "              <div class='form-check'>\n";
					choiceHTML += "   			 <form method='POST' action='/Compete?Q_ID="+ (nextQuestionID) +"'>\n";
					choiceHTML += "                  <label class='form-check-label'> &nbsp;\n";
					
					if ( correctAnswer_Choice_ID == currentChoiceID )
					{
						choiceHTML += "                      <input type='radio' class='form-check-input correct' name='C_ID' value='"+ choiceResultSet.getObject("id") +"'>\n";
						choiceHTML += "                      <input type='hidden' name='isCorrect' value='True'>\n";
					}
					else
					{
						choiceHTML += "                      <input type='radio' class='form-check-input incorrect' name='C_ID' value='"+ choiceResultSet.getObject("id") +"'>\n";
						choiceHTML += "                      <input type='hidden' name='isCorrect' value='False'>\n";
					}
					choiceHTML += "                              &nbsp;" + currentChoiceText + "\n";
					choiceHTML += "                      <input type='hidden' name='selectedChoiceID' value='" + currentChoiceID + "'>\n";
					choiceHTML += "					<input type='hidden' name='prevQuestionID' value='" + currentQuestionID + "'>" ;// used to insert into Submission t\na;
					choiceHTML += "					<input type='hidden' name='PointValue' value='" + currQuestionPointValue + "'>\n";
					choiceHTML += "					<input type='hidden' name='username'   value='" + username + "'>\n";
					choiceHTML += "                  </label>\n";
					choiceHTML += "   			 </form>\n";
					choiceHTML += "              </div>\n";
					choiceHTML += "          </td>\n";
			...
			// Query Scores
			String retreiveScoreQuery = "SELECT * FROM Math.Competitor ORDER BY score DESC";
